...
typedef struct {
    ...
    rmt_uart_context_tx_t rmt_uart_context_tx;
    rmt_uart_context_rx_t rmt_uart_context_rx;
    uint16_t rmt_bit_len;
    bool configured;
} rmt_uart_context_t;

static rmt_uart_context_t rmt_uart_context[RMT_UART_NUM_MAX] = {};

static int convert_byte_to_items(rmt_uart_context_t* ctx, uint8_t byte)
{
    rmt_uart_context_tx_t* rtc = &ctx->rmt_uart_context_tx;
...
static int convert_data_to_items(rmt_uart_context_t* ctx, const uint8_t* data, uint16_t len)
{
    rmt_uart_context_tx_t* rtc = &ctx->rmt_uart_context_tx;
    rtc->item_index = 0;
...
static void fill_bits(rmt_uart_context_t* ctx, uint32_t duration, uint32_t level)
{
    rmt_uart_context_rx_t* rrc = &ctx->rmt_uart_context_rx;
    uint32_t rmt_bit_numbers = round_closest(duration, ctx->rmt_bit_len);
    ...
    uint32_t from = rrc->bit_num;
    uint32_t to = rrc->bit_num + rmt_bit_numbers;
...
esp_err_t rmt_uart_init(rmt_uart_port_t uart_num, const rmt_uart_config_t* uart_config)
{
    ...
    rmt_uart_context[uart_num].rmt_bit_len = bit_len;
    memcpy(&rmt_uart_context[uart_num].rmt_uart_config, uart_config, sizeof(rmt_uart_config_t));

    if (uart_config->mode == RMT_UART_MODE_RX_ONLY || uart_config->mode == RMT_UART_MODE_TX_RX)
    {
        rmt_config_t rmt_config_rx = RMT_DEFAULT_CONFIG_RX(uart_config->rx_io_num, (rmt_channel_t)uart_num);
        ...
        rmt_uart_context[uart_num].rmt_config_rx = rmt_config_rx;
        rmt_get_ringbuf_handle(rmt_config_rx.channel, &rmt_uart_context[uart_num].rmt_uart_context_rx.rb);
        ESP_RETURN_ON_FALSE((rmt_uart_context[uart_num].rmt_uart_context_rx.rb), ESP_FAIL, TAG, "rmt ringbuffer is null");
#if CONFIG_SPIRAM_USE_MALLOC
        rmt_uart_context[uart_num].rmt_uart_context_tx.items = (rmt_item32_t *)heap_caps_calloc(1, uart_config->buffer_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
#else
        rmt_uart_context[uart_num].rmt_uart_context_tx.items = calloc(1, uart_config->buffer_size);
#endif
        rmt_rx_start(rmt_uart_context[uart_num].rmt_config_rx.channel, true);
    }

    if (uart_config->mode == RMT_UART_MODE_TX_ONLY || uart_config->mode == RMT_UART_MODE_TX_RX)
    {
        rmt_config_t rmt_config_tx = RMT_DEFAULT_CONFIG_TX(uart_config->tx_io_num, (rmt_channel_t)(uart_num + 1));
        ...
        rmt_uart_context[uart_num].rmt_config_tx = rmt_config_tx;
    }

    rmt_uart_context[uart_num].configured = true;
...
{
    rmt_uart_context_t* ctx = &rmt_uart_context[uart_num];
    ...
    return rmt_write_items(ctx->rmt_config_tx.channel, ctx->rmt_uart_context_tx.items, rtc->item_index, true);
}
...
esp_err_t rmt_uart_deinit(rmt_uart_port_t uart_num)
{
    ...
    if (ctx->rmt_uart_config.mode != RMT_UART_MODE_RX_ONLY)
    {
        ...
        free(rtc->items);
#endif
        ret = rmt_driver_uninstall((rmt_channel_t)(uart_num + 1));
        if (ret != ESP_OK) return ret;
    }

    if (ctx->rmt_uart_config.mode != RMT_UART_MODE_TX_ONLY)
    {
        ret = rmt_driver_uninstall((rmt_channel_t)uart_num);
        if (ret != ESP_OK) return ret;
    }
